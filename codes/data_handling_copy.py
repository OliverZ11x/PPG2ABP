...
def fold_data():
    ...
    for fold_id in tqdm(range(1), desc='Folding Data'):        # iterate for 10 folds

        fl = h5py.File(os.path.join('data', 'ARdata.hdf5'), 'r')      # load the episode data
        ...
        for i in tqdm(range(0, 100), desc='Training Data Part 1'):    # training samples before validation samples
        ...
        for i in tqdm(range(100, 110), desc='Training Data Part 2'):    # training samples after validation samples
        ...
        for i in tqdm(range(110, 120), desc='Validation Data'):
    ...
    fl = h5py.File(os.path.join('data', 'ARdata.hdf5'), 'r')      # loading the episode data

    X_test = []                 # intialize test data
    Y_test = []

    for i in tqdm(range(120, len(fl['data']))):
    ...
    pickle.dump({'max_ppg': max_ppg,
                'min_ppg': min_ppg,
                'max_abp': max_abp,
                'min_abp': min_abp}, open(os.path.join('data', 'meta{}.p'.format(9)), 'wb'))
